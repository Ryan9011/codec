...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // HISTORICAL DATA
            ...
            data.report_type = readHistoryEvent(report_type & 0x07);
            data.occupancy = readOccupancyStatus(bytes[i + 5]);
            data.temperature = readInt16LE(bytes.slice(i + 6, i + 8)) / 10;
            data.humidity = readUInt8(bytes[i + 8]) / 2;
            i += 9;
...
function handle_downlink_response(channel_type, bytes, offset) {
    ...
    switch (channel_type) {
        ...
        case 0x8e:
            ...
            break;
        case 0x95:
            decoded.vacant_reporting_interval = readUInt16LE(bytes.slice(offset, offset + 2));
            offset += 2;
            break;
        case 0x96:
...
function readTemperatureAlarm(status) {
    var status_map = { 0: "threshold alarm release", 1: "threshold alarm" };
    return getValue(status_map, status);
}

function readHistoryEvent(status) {
    var status_map = { 0: "temperature threshold alarm release", 1: "temperature threshold alarm", 2: "pir idle", 3: "pir trigger", 4: "period" };
    return getValue(status_map, status);
}
...
function readD2DMode(status) {
    var mode_map = { 1: "occupied and temperature threshold alarm", 2: "occupied", 3: "vacant", 4: "temperature threshold alarm", 5: "temperature threshold alarm release" };
    return getValue(mode_map, status);
}
